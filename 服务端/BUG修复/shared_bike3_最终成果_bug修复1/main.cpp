...
int main(int argc , char ** argv)
{
	...
	if (!Logger::instance()->init("/home/wxncom/projects/conf/log.conf")/*!Logger::instance()->init(std::string(argv[2]))*/)
	{
		fprintf(stderr,"init log module failed.\n");
		return -2;
	}

	Iniconfig *config = Iniconfig::getInstance();//单例模式
	if (!config->loadfile("/home/wxncom/projects/conf/shared_bike.ini")/*!config->loadfile(std::string(argv[1]))*/)
	{
		//printf("load %s failed\n",argv[1]);
		LOG_ERROR("load %s failed.\n", "/home/wxncom/projects/conf/shared_bike.ini");
	...
	LOG_DEBUG("<mian.cpp> 调用网络接口类的start方法...");
	NTF->start(conf_args.svr_port);////////////////////conf_args.svr_port
	LOG_DEBUG("<mian.cpp> 网络接口类的start方法调用结束...");

	printf("Wainting to connect to the client...\n");
	LOG_DEBUG("<mian.cpp> Wainting to connect to the client...");
	while (1 == 1)
	{
