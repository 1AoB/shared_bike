...
void NetworkInterface::listener_cb(evconnlistener* listener, evutil_socket_t fd, sockaddr* sock, int socklen, void* arg)
{
	...
	ConnectSession* cs = session_init(fd, bev);/////////////////这里是否正确?????????????????////////////////////
	...
	/*
	* bufferevent_setcb 是一个函数，它用于设置 bufferevent 对象的回调函数。 bufferevent 是一个提供缓冲区读写和网络事件通知的抽象网络 I/O 对象
	* bev：指向 bufferevent 对象的指针。
	* readcb：读事件回调函数指针，当有数据可读时调用。
	* writecb：写事件回调函数指针，当可以写入数据时调用。
	* eventcb：事件回调函数指针，当发生网络事件时调用。
	* cbarg：回调函数的参数指针，可以是任何类型的指针，用于向回调函数传递额外的参数。
	*/
	bufferevent_setcb(bev, handle_request, handle_response, handle_error, cs);/////////////线索12,找handle_request的参数cs////////////////////
...
void NetworkInterface::handle_request(bufferevent* bev, void* arg)////////////线索11,arg可能有问题///////////////////////////
{
	...
	if (cs->session_stat != SESSION_STATUS::SS_REQUEST)
	{
		...
		//报喜
		LOG_DEBUG("NetworkInterface::handle_request right session stat[%d]\n", cs->session_stat);
	}
	...
	//read_message[len] = '\0';
	//DispatchMsgService::addClientNumber();
	//LOG_DEBUG("read message from client[%p]: %s", bev, read_message);//FBEB
	...
	if (cs->req_stat == MESSAGE_STATUS::MS_READ_MESSAGE && evbuffer_get_length(bufferevent_get_input(bev)) > 0)
	{
		...
		if (cs->read_message_len == cs->message_len)
		{
			...
			printf("!!!cs->read_buf: [%s]\n", cs->read_buf);/////////////////////线索10,cs->read_buf不为空,但是开头多了一个换行,找write_buf////////////////
