...
iEvent* UserEventHandler::handle(const iEvent* ev)//////////////////线索3//////////////////////////////////
{
	u32 eid = ev->get_eid();
	...
	case EEVENTID_SCANQRSTART_REQ:
		return handle_bike_scanQRSatrt_req((bikeScanQRStartReqEv*)ev);////////////////////////线索2//////////////////////////////
		break;
...
bikeScanQRStartResponseEv* UserEventHandler::handle_bike_scanQRSatrt_req(bikeScanQRStartReqEv* ev)
{
	...
	//更新用户骑行记录表
	printf("<user_event_handler.cpp>_handle_bike_scanQRSatrt_req : %s", ev->get_userName().c_str());///////////////线索1///////////////////////////////////
	
	/*sprintf(sql_content_ride_record, "insert into user_ride_record_info(username,bikeId,bikeStatus,start_time, start_point) \
		values('15690578178', %d, %d, FROM_UNIXTIME(%d, '2023-07-13 0:26:45'),GeomFromText('POINT(%lf %lf)'))",
		ev->get_bikeId(), BIKE_RIDE_START, ev->get_timestamp(), ev->get_longitude(), ev->get_latitude());*/
	
	sprintf(sql_content_ride_record, "insert into user_ride_record_info(username,bikeId,bikeStatus,start_time, start_point) values(\"%s\", %d, %d, FROM_UNIXTIME(%d, '%%Y-%%m-%%d %%H:%%i:%%S'),GeomFromText('POINT(%lf %lf)'))",
		ev->get_userName().c_str(), ev->get_bikeId(), BIKE_RIDE_START, ev->get_timestamp(), ev->get_longitude(), ev->get_latitude());
	
	LOG_DEBUG("<user_event_handler.cpp>sql_content:[%s]", sql_content_ride_record);

	//执行sql语句
	if (!us.insertUseRideRecord(sql_content_ride_record))
	{
		LOG_DEBUG("<user_event_handler.cpp>用户骑行记录插入user_ride_record_info失败！");
		LOG_DEBUG("<user_event_handler.cpp>错误信息：%s", mysqlconn->GetErrInfo());
		return new bikeScanQRStartResponseEv(ERRO_PROCCESS_FALED, 0, 0, 0);
	}
